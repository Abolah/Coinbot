...
class Order():
    ...
    def order(self,price,profit,loss
        ...
        embed.add_field(name=":star2: Requête on the orders of " + self.auth,
                        value="Here are the information that I could retrieve" +self.auth,
                        inline=False)
        embed.add_field(name=":chart_with_upwards_trend: Information about your take profit", value=data_achat, inline=True)
        embed.add_field(name=":chart_with_downwards_trend: Information about your stop loss", value=data_vente, inline=True)
