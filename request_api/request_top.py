...
class Topcoin():
    ...
    def to_list(self,json_data):
        list = []
        try:
            for i in json_data:
                if i["percent_change_24h"] == None:
                    i["percent_change_24h"] = 0
                if i["percent_change_1h"] == None:
                    i["percent_change_1h"] = 0
                if i["percent_change_7d"] == None:
                    i["percent_change_7d"] = 0
                list.append([i["name"],int(i["rank"]),float(i["percent_change_1h"]),float(i["percent_change_24h"]),float(i["percent_change_7d"]),i["symbol"]])
        except Exception as e:
            print("Error converting to list")
            return "Error converting to list" + str(e)
        return list

    def top_sorting(self,list):
    ...
    def format_affichage(self,sorted_list,data_global,author):
        ...
        try:
            ...
            author = "khey"
        try:
            wcoin1= "Rank [" + str(sorted_list[-1][1]) + "] [" + sorted_list[-1][5] + "] " + sorted_list[-1][0] + " : " + str(sorted_list[-1][3])+ " %\n"
            wcoin2 = "Rank [" + str(sorted_list[-2][1]) + "] [" + sorted_list[-2][5] + "] " + sorted_list[-2][0] + " : " + str(sorted_list[-2][3]) + " %\n"
            wcoin3 = "Rank [" + str(sorted_list[-3][1]) + "] [" + sorted_list[-3][5] + "] " + sorted_list[-3][0] + " : " + str(sorted_list[-3][3]) + " %\n"
            wcoin4 = "Rank [" + str(sorted_list[-4][1]) + "] [" + sorted_list[-4][5] + "] " + sorted_list[-4][0] + " : " + str(sorted_list[-4][3]) + " %\n"
            wcoin5 = "Rank [" + str(sorted_list[-5][1]) + "] [" + sorted_list[-5][5] + "] " + sorted_list[-5][0] + " : " + str(sorted_list[-5][3]) + " %\n"
            value_win = "```css\n" + wcoin1 + wcoin2 + wcoin3 + wcoin4 + wcoin5 + "```"
        except Exception as e :
            print(e)
            value_win = "```css\nErreur de l'API CMC : Principal win```"
        try:
            lcoin1 = "Rank [" + str(sorted_list[0][1]) + "] [" +sorted_list[0][5] + "] " + sorted_list[0][0] + " : " + str(sorted_list[0][3]) + " %\n"
            lcoin2 = "Rank [" + str(sorted_list[1][1]) + "] [" + sorted_list[1][5] + "] " + sorted_list[1][0] + " : " + str(sorted_list[1][3]) + " %\n"
            lcoin3 ="Rank [" + str(sorted_list[2][1]) + "] [" + sorted_list[2][5] + "] " + sorted_list[2][0] + " : " + str(sorted_list[2][3]) + " %\n"
            lcoin4 = "Rank [" + str(sorted_list[3][1]) + "] [" + sorted_list[3][5] + "] " + sorted_list[3][0] + " : " + str(sorted_list[3][3]) + " %\n"
            lcoin5 = "Rank [" + str(sorted_list[4][1]) + "] [" + sorted_list[4][5] + "] " + sorted_list[4][0] + " : " + str(sorted_list[4][3]) + " %\n"
