...
class Stats:
    ...
    def create(conn):
        ...
        c = conn.cursor()
        try:
            c.execute('''CREATE TABLE IF NOT EXISTS stats (id INTEGER PRIMARY KEY , command TEXT, coin TEXT)''')
            conn.commit()
            create = 0
        ...
        if arg is None:
            ...
            for i in range(0, len(list_command)):
                if i > 4:
                    ...
                    value_command = value_command + "[Rank " + str(i + 1) + "] " + str(
                        list_command[i][0]) + " has been asked " + str(list_command[i][1]) + " time\n"
            ...
            for i in range(0, len(list_coin)):
                if i > limit:
                    ...
                    if list_coin[i][0] == "":
                        ...
                        value_coin = value_coin + "[Rank " + str(rank) + "] " + str(
                            list_coin[i][0]).upper() + " has been asked " + str(list_coin[i][1]) + " time \n"
                        rank += 1
            result_command = "```css\n" + value_command + "```"
            result_coin = "```css\n" + value_coin + "```"
            ...
            result = "```css\nCoin " + arg.upper() + " has been asked " + str(data[0][0]) + " times on the bot```"
